...
| `UpgradeManager.buyTree` (M13) | the `tree:<id>` unlock that gates one tree's nodes, on the Forge itself | `UNKNOWN_ID`, `ALREADY_OWNED`, `NOT_FOR_SALE`, `INSUFFICIENT_FUNDS` — and `OK` at cost `0` when the tree was already earned by play (`isTreeEarned`) |
...
| --- | --- | --- |
| `BirdSelectionScreen` | the browsed bird as the hero — name, archetype line, three `AttributeBadge`s built by `BirdAttributes.of` from the bird's own profile and innate abilities — over a `Carousel` of the seven birds, each with a procedural portrait in the selected palette, the archetype, and, for a locked one, the **cheapest** way to open it in words or coins; the palette swatches with their conditions; a run-setup bar whose panel holds the world row, the tier picker with the locked tiers marked (E19) and the run-mode row (§8) — Standard, Seeded and, when the screen has a clock, Daily, each saying what it would play, with the world and tier read-only under a settled daily; the loadout as `AbilityCard`s — one active, one per passive slot the bird and the `passive_slot` grant give, and a fixed card per innate passive — with the ability panel and the stat breakdown of the run that would start right now behind **See details**; and one gold call to action (Use / Bird selected / Buy / Locked) over the hub's five-item `SectionNav` | `SelectionManager` (bird, palette, tier, loadout), `UnlockManager` (Buy), `UpgradeManager.buyAbilityLevel`, and the mode row's write of the daily pick (E27) |
| `UpgradeTreeScreen` — the **Forge** (rebuilt M13) | five bands: a compact header with the title, the open tree's subtitle, the wallet in its coin pill and the hub's forge scene drawn small; the three tree tabs (flight, economy, forge) with a glyph each and the selected one in gold; a scrolling tree viewport where every node is a row card laid out by tier under a green tier pill, with the prerequisite edges drawn as light-yellow elbows from the shipped `prereqs` graph and each card carrying level/maximum, what one level does in words and its state (tree locked / prerequisite missing / affordable / maxed); a detail panel with the selected node's hero glyph, name, level, description, status in words and one gold call to action — *UPGRADE • n* for a node, *UNLOCK TREE • n* for a locked tree, *MAXED* at the cap — then the attribute summary: every stat the tree can touch with its live resolved value and a five-segment pip bar over the stat's clamp range; and the hub's five-item `SectionNav` with Forge on the gold plate | `UpgradeManager.buy` for a node, `UpgradeManager.buyTree` for a locked tree — and `UpgradeManager.claimEarnedTrees` on entry, which gives away a tree already earned by play instead of selling it back |
| `ShopScreen` (M4, rebuilt M12) | everything with a `purchase` branch — owned or not — grouped into four tabs (birds, abilities, worlds, features) on a scrolling two-column grid of `ShopCard`s, each with the thing's own art (`ShopArt`), its name, the road that costs no coins as its second line (`ProgressionText.unlockText` over `UnlockEvaluator.nearestBranch` preferring the earnable branch) and its state on a wooden tag: the price, a check and *Owned*, an ability's level and next price, or the cap word. What is for sale comes first, cheapest first, the owned tail last; an unaffordable card is dimmed under a crisp tag and never padlocked, because everything here is also earnable. A wooden plaque carries the description, the free road and the next level's effect — never the price — and one gold call to action names the tab's verb. The hub's `HubHeader` shows the wallet once and the `SectionNav` puts Shop on the gold plate; an offer that is not playable yet says which milestone it arrives in | `UnlockManager.purchase`, `UpgradeManager.buyAbilityLevel` |
...
Iron Forge world, the `forge` bird archetype (Cinder) or the `master_forge_1` node. Since M13 the
screen it opens carries that name too — *Forge*, with one subtitle per tree — where it used to be
titled "Upgrades".
...
| `BirdSelectionScreenTest` | the roster, the cheapest path in words, selection writing and saving, a purchase that moves the wallet and one that cannot, the breakdown against `StatSheet.breakdown`, the M5 loadout row (the cards a bird's slots and the `passive_slot` grant give, cycling one, and that cycling never drops a passive the bird cannot show), and the M11 shell: what the call to action says in each of its four states, the two panels opening and closing, the hero following the browsed tile, a locked tile nudging instead of selecting, and every control on the ring in visual order |
...
| `ShopScreenTest` | the four tabs, cheapest first, affordability, a purchase turning its card into an owned one that sorts to the end, the three modifier legendaries at 300 coins each, the milestone note on what is not playable yet, and the M12 shell: the header's wallet as a readout, Shop on the gold plate, four uniform tabs with a glyph each, the verb of the call to action per tab and its silence once owned, the plaque never repeating the price, the grid scrolling and clamping with a card below the band not answering the pointer, the language switch, and `Esc` leaving with no Back button |
