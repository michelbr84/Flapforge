...
| `src/test/java/io/github/michelbr84/flapforge/GameScreenTest.java` | ground scroll rate, freeze on death, the sky under the game-over overlay |
| `src/test/java/io/github/michelbr84/flapforge/render/HudRendererTest.java` | the READY-hint blink period |
...
| start / game-over prompt hidden while `flashCount ≤ 30`, shown until 60, then reset (1 s off, 1 s on) | `HudRenderer` READY hint: **60 ticks hidden, 60 ticks shown**; the game-over strip switched from a blinking key hint to retry/summary/menu buttons, so only the READY hint blinks | `theReadyHintBlinksOnTheUpstreamPeriod` (HudRendererTest) | the buttons make the strip usable by touch, where no keyboard exists |
...
| `Frame.setSize(420, 640)` counted the window **insets**, so roughly 28 px of the playfield were hidden under the title bar — which is why upstream needed `TOP_BAR_HEIGHT 20` to keep clouds out of it (E30.i) | the window sizes its **canvas** to 420×640 and the viewport letterboxes it, so all 640 rows are visible; on taller windows the renderers additionally paint cosmetic sky/earth beyond rows 0/640 (`render.Overscan`, `settings.fillScreen`) — never gameplay; fullscreen saves and restores the windowed bounds and decoration state | `WindowScaleTest`, `SmokeWindowTest`, `OverscanRenderTest` | `CloudLayer.SPAWN_TOP_Y` keeps the 20 px band anyway, for the same look |
