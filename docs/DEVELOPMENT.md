...
| `./gradlew fatJar` | self-contained `build/libs/flapforge-<version>-all.jar` (Gson bundled; `0.2.0` ships `flapforge-0.2.0-all.jar`) |
...
1. **The version lives in `src/main/resources/version.properties`** (`version=0.2.0`); Gradle
   reads it, so the plain jar's `Implementation-Version`, the fat jar's file name
   (`build/libs/flapforge-0.2.0-all.jar`) and the `jpackage` app version all follow it. A
...
| `--world ID` | select the given world for this launch (`green_fields`, `wind_valley`, `iron_forge`, `storm_sky`, `void`). Not an unlock: when the profile owns the world the selection is written (as the World Select would) and the hub's plaque and START RUN play it; when it does not, the profile is left alone and a line on stdout says `--world <id>: not unlocked in this profile (world:<id>); the selection stays <owned> and START RUN plays the selection` — since M10 the hub builds its own run factory over the live selection, so a locked world cannot be flown from the hub at all. An unknown id is reported on stderr and ignored. With `--headless-run` the hash line is computed in that world (without the flag it is the classic configuration, so the published hash is untouched). | M7 (M10: hub) |
